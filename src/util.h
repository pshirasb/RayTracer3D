...
enum MatType { DIELECTRIC, DIFFUSE };

struct Material {
	Material( MatType mattype, Colour ambient, Colour diffuse, Colour specular, 
              double exp, double g ) :
		type(mattype),
        ambient(ambient), diffuse(diffuse), specular(specular), 
		exp(exp), gloss(g) {}
	
   // Material type used to calulate shading
    MatType type; 
	...
	// In diffuse Mat   : Specular components for Phong shading.
	// In dielectric Mat: Linear attenuation coefficient in Beer's Law
	Colour specular;

	// In diffuse Mat    : Specular exponent.
	// In dielectric Mat : Index of refraction
	double exp;
    
    // In dielectric Mat : Glossiness [0,1]
    double gloss;
