...
public:
    ...
    // Add a SceneDagNode* with an empty obj and six UnitSquares
    // as children that form a UnitBox, to _root.
    SceneDagNode* addBox(Material* mat);
...
private:
    ...
    // Return the colour of the ray after intersection, shading and
    // shadow calculations.
    Colour traceShadow(LightSource *light, Ray3D& ray, int depth); 

    // Add reflection to the colour of the ray depth times 
    // recursively, if the intersected point lies
    // on a dielectric material.
    Colour traceReflection(Ray3D& ray, int depth);

    // Add refraction to the colour of the ray using
    // Schlick's approximation to Fresnel's equations and Beer's Law,
    // if the intersected point lies on a dielectric material.
    Colour traceRefraction(Ray3D& ray, int depth);
	...
	void computeShading( Ray3D& ray, int depth );
	
    void render_thread(process_data *pd);
    ...
    static const int _max_depth = 6;

    // Number of anti-aliasing samples
    static const int _aa_samples = 1;    

    // Number of shadow samples
    static const int _shadow_samples = 4;    
    
    // Number of gloss samples
    static const int _gloss_samples = 1;    
    
    // Number of depth of field samples
    static const int _dof_samples = 1;    
    
    // Number of threads
    static const int _max_threads = 4;
